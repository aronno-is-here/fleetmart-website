...
function SocialButtons() {
  ...
  const handleAppleLogin = async () => {
    try {
      if (window.AppleID?.auth) {
        ...
        dispatch(toast({ type: 'info', message: 'Apple Sign-In requires Apple JS SDK. Use email/password for now.' }))
      }
    } catch {
  ...
  const fallbackGoogle = () => {
    dispatch(toast({ type: 'info', message: 'Google Sign-In requires a valid Client ID. Please use email/password.' }))
  }
  ...
  return (
    <div className="space-y-3">
      {import.meta.env.VITE_GOOGLE_CLIENT_ID ? (
        ...
        <button type="button" onClick={fallbackGoogle} className="flex w-full items-center justify-center gap-3 border border-line bg-pitch2 px-4 py-3 text-sm font-medium text-chalk transition-all duration-200 hover:border-chalk hover:bg-pitch">
